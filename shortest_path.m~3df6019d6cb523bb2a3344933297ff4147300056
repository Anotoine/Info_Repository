...
if closest_node (g,a)==b
    found=true;
    pts(1).points(2).node=nStart;
    pts(1).points(2).weight=direct_cost(g,a,b);
    porDonde=p;
    if verbose==true
    print_path(g,pts(1));
    end
...
x=1;
    ...
    if encontrado==false
      r=node_name(g,V(n));
      pts{length(pts)+1}=add_node_to_path(porDonde,r,cost);
    end
    encontrado=false; 
    x=1;
    n=n+1;
