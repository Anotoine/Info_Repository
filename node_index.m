...
while((found==false)&&(n<=length(g.node)))   %Mira todos los nombres y los compara con el nombre introducido  (Search)
    if(node_name==g.node(n).name)    %Si el nombre concuerda con alguno de los nombres de los vectores, entonces se sale del Search y se define index como el valor que corresponde en el vector
        found=true;
        index=n;
    end
    n=n+1;
